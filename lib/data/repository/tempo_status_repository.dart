...
class TempoStatusRepositoryImpl implements TempoStatusRepository {
  ...
  Future<TempoStatus?> getMyStatus(String userId) async {
    try {
      final doc =
          await _firestore.collection('tempoStatuses').doc(userId).get();
  ...
  Future<void> deleteStatus(String userId) async {
    try {
      await _firestore.collection('tempoStatuses').doc(userId).delete();
    } catch (e) {
  ...
  @override
  Future<List<TempoStatus>> getNearbyStatuses(String geohash,
      {int limit = 20}) async {
    try {
  ...
  @override
  Future<List<TempoStatus>> getStatusesByMood(String mood,
      {int limit = 20}) async {
    try {
      final now = Timestamp.now();
      ...
      return query.docs.map((doc) => TempoStatus.fromJson(doc.data())).toList();
    } catch (e) {
  ...
  @override
  Future<List<TempoInteraction>> getMyStatusInteractions(
      String statusOwnerId) async {
    try {
      final query = await _firestore
          .collection('tempoInteractions')
          .doc(statusOwnerId)
  ...
  Future<void> cleanupOldInteractions() async {
    try {
      ...
      final interactionDocs =
          await _firestore.collection('tempoInteractions').get();
