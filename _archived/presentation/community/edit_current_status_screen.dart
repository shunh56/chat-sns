...
class EditCurrentStatusScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final listView = asyncValue.when(
      data: (me) {
        final nowWithUsers = ref
            .read(allUsersNotifierProvider)
            .asData!
            .value
            .values
            .where((user) => currentStatus.nowWith.contains(user.userId))
            .toList();
        return Padding(
          padding:
...
*/
              ...
              nowWithUsers.isEmpty
                  ? GestureDetector(
                      onTap: () {
                        ref.read(selectedUserIdsStateProvider.notifier).state =
                            currentStatus.nowWith;
                        Navigator.push(
                          context,
                          MaterialPageRoute(
